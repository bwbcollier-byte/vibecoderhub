...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  return MARKETPLACES.items.map((m) => ({ slug: m.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const m = MARKETPLACES.items.find((x) => x.slug === slug);
...
export default async function Page({ params }: PageProps): Promise<ReactElement> {
  const { slug } = await params;
  const resource = MARKETPLACES.items.find((m) => m.slug === slug);
  if (!resource) notFound();
  const alternatives = MARKETPLACES.items.filter((m) => m.slug !== resource.slug).slice(0, 4);
  return (
