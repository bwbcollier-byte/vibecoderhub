...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  const slugs = await listResourceSlugs(SUBAGENTS.config.typeId);
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const m = await getResourceBySlug(SUBAGENTS.config.typeId, slug);
...
export default async function Page({ params }: PageProps): Promise<ReactElement> {
  const { slug } = await params;
  const resource = await getResourceBySlug(SUBAGENTS.config.typeId, slug);
  if (!resource) notFound();
  const all = await listResources(SUBAGENTS.config.typeId);
