...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  return WORKFLOWS.items.map((m) => ({ slug: m.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const m = WORKFLOWS.items.find((x) => x.slug === slug);
...
export default async function Page({ params }: PageProps): Promise<ReactElement> {
  const { slug } = await params;
  const resource = WORKFLOWS.items.find((m) => m.slug === slug);
  if (!resource) notFound();
  const alternatives = WORKFLOWS.items.filter((m) => m.slug !== resource.slug).slice(0, 4);
  return (
