...
export default async function Page(): Promise<ReactElement> {
  const [items, totalCount] = await Promise.all([
    listResources(SHOWCASE.config.typeId),
    getResourceCount(SHOWCASE.config.typeId),
  ]);
  return <ResourceIndexPage items={items} config={SHOWCASE.config} totalCount={totalCount} />;
}
