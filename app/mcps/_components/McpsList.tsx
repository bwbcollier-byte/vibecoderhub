...
interface McpsListProps {
  initialMcps: McpDetail[];
}

const SORT_OPTIONS: { value: McpSort; label: string }[] = [
...
export function McpsList({ initialMcps }: McpsListProps): React.ReactElement {
  ...
  const filtered = React.useMemo(
    () => sortMcps(filterMcps(initialMcps, query, client), sort),
    [initialMcps, query, sort, client],
  );
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="relative md:w-[360px]">
          ...
          <Input
            type="search"
            placeholder="Search MCPs, authors, capabilities…"
            value={query}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1 flex-wrap" role="radiogroup" aria-label="Client filter">
      ...
      <div className="font-mono uppercase tracking-[1.4px] text-[10px] font-bold text-text-secondary">
        {filtered.length} {filtered.length === 1 ? 'MCP' : 'MCPs'}
      ...
      {hydrating ? (
        ...
        <EmptyState
          glyph="∅"
          title="No MCPs match"
          body="Try clearing search, the client filter, or the sort — there are many more once Supabase is wired up."
        />
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
